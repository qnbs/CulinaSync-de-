...
/** Domain data (Vorrat, Rezepte, …) — getrennt von `CulinaSyncStateDB` (Redux-Persist). */
export const PRIMARY_DB_NAME = 'CulinaSyncDataDB';
export const LEGACY_PRIMARY_DB_NAME = 'CulinaSyncDB';
...
/** Kopiert Daten von der älteren IndexedDB `CulinaSyncDB` nach `CulinaSyncDataDB` und entfernt die alte DB. */
export const migrateLegacyPrimaryDatabaseIfNeeded = async (): Promise<void> => {
  if (typeof indexedDB === 'undefined') {
    return;
  }

  const existsNew = await Dexie.exists(PRIMARY_DB_NAME);
  if (existsNew) {
    return;
  }

  const existsOld = await Dexie.exists(LEGACY_PRIMARY_DB_NAME);
  if (!existsOld) {
    return;
  }

  const oldDb = new Dexie(LEGACY_PRIMARY_DB_NAME);
  applyDbMigrations(oldDb);
  await oldDb.open();

  const newDb = new Dexie(PRIMARY_DB_NAME);
  applyDbMigrations(newDb);
  await newDb.open();

  const tableNames = ['pantry', 'recipes', 'mealPlan', 'shoppingList', 'appLogs'] as const;
  for (const tableName of tableNames) {
    if (!oldDb.tables.some((t) => t.name === tableName)) {
      continue;
    }
    const rows = await oldDb.table(tableName).toArray();
    if (rows.length > 0) {
      await newDb.table(tableName).bulkPut(rows);
    }
  }
