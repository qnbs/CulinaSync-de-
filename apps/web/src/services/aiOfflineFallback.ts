...
const normalizeToken = (value: string) =>
  value.toLowerCase().replace(/ae/g, 'a').replace(/oe/g, 'o').replace(/ue/g, 'u').replace(/\u00df/g, 'ss');
...
const getPantryHighlights = (pantryItems: PantryItem[], limit = 5) =>
  pantryItems
    .slice()
...
export const shouldUseOfflineFallback = (error: unknown) => {
...
// QNBS-v3: Gemeinsamer i18n-Offline-Fallback für aiService und geminiService (kein Faker/Englisch-Leak)
export const buildLocalRecipeIdeas = (
...
export const buildLocalShoppingList = (
  prompt: string,
  pantryItems: PantryItem[],
