...
const PantryListItem = React.memo<PantryListItemProps>(({
    ...
    else if (isSelected) indicatorClasses += 'border-[var(--color-accent-500)] bg-[var(--color-accent-500)]/10';
        ...
        <li className={`grid ${isSelectMode ? 'grid-cols-[auto_1fr_auto]' : 'grid-cols-[1fr_auto]'} gap-x-3 items-center p-3 group ${indicatorClasses}`}>
            {isSelectMode && (
              <button onClick={() => onToggleSelect(item.id!)} className="p-1 text-zinc-400" aria-label={`Select ${item.name}`}>
                {isSelected ? <CheckSquare className="text-[var(--color-accent-400)]"/> : <Square/>}
              </button>
            )}
            ...
            >
              <div className="flex items-center gap-2">
                  ...
                  {(isRunningLow || status !== 'ok') && <span title={status === 'expired' ? 'Abgelaufen!' : (status === 'nearing' ? `Läuft in ${pantrySettings.expiryWarningDays} Tagen oder weniger ab!` : `Wird knapp! Mindestmenge: ${item.minQuantity} ${item.unit}`)}><AlertTriangle size={14} className={status === 'expired' ? 'text-red-400' : 'text-yellow-400'}/></span>}
              </div>
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-200">
              {isRunningLow && <button onClick={() => onAddToShoppingList(item)} aria-label={`Add ${item.name} to shopping list`} className="p-2 rounded-full text-zinc-400 hover:bg-zinc-700 hover:text-[var(--color-accent-400)]"><ShoppingCart size={16} /></button>}
