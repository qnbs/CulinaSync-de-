...
// Improved heuristic for category mapping (labels follow current UI language)
export const getCategoryForItem = (itemName: string): string => {
    const settings = loadSettings();
    if (!settings.shoppingList.autoCategorize) {
        return i18next.t('shoppingList.categories.misc');
    }

    const name = itemName.toLowerCase();
    if (['milch', 'joghurt', 'käse', 'butter', 'sahne', 'quark', 'feta', 'ei', 'eier'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.dairy');
    if (['apfel', 'banane', 'tomate', 'zwiebel', 'knoblauch', 'karotte', 'salat', 'paprika', 'spinat', 'gemüse', 'kartoffel', 'gurke', 'zucchini', 'lauch', 'petersilie', 'basilikum', 'schnittlauch'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.produce');
    if (['huhn', 'rind', 'schwein', 'fisch', 'lachs', 'fleisch', 'wurst', 'hackfleisch'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.meat');
    if (['brot', 'brötchen', 'baguette', 'toast'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.bakery');
    if (['nudeln', 'reis', 'mehl', 'zucker', 'linsen', 'bohnen', 'pasta', 'haferflocken', 'müsli', 'kaffee', 'tee', 'couscous'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.dryGoods');
    if (['öl', 'essig', 'olivenöl'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.oils');
    if (['tomatenmark', 'passata', 'kichererbsen', 'mais', 'oliven', 'konserve'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.canned');
    if (['salz', 'pfeffer', 'paprika', 'curry', 'senf', 'ketchup', 'sojasauce', 'brühe'].some(s => name.includes(s))) return i18next.t('shoppingList.categories.spices');
    return i18next.t('shoppingList.categories.misc');
}
