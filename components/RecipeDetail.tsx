...
const RecipeDetail: React.FC<RecipeDetailProps> = ({ recipe, onBack }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      if(newId) {
        recipe.id = newId; // Update recipe object with ID
        setIsSaved(true);
  ...
  const handleDelete = async () => {
    if (recipe.id && window.confirm(`Möchtest du das Rezept "${recipe.recipeTitle}" wirklich endgültig löschen?`)) {
      try {
  ...
  const handleToggleFavorite = async () => {
    if (recipe.id) {
        try {
            ...
            await db.recipes.update(recipe.id, { isFavorite: newIsFavorite });
  ...
  const allTags = recipe.tags ? [
    ...(recipe.tags.course || []),
    ...(recipe.tags.cuisine || []),
    ...(recipe.tags.occasion || []),
    ...(recipe.tags.mainIngredient || []),
    ...(recipe.tags.prepMethod || []),
  ...
  return (
    <div className="animate-fade-in">
      {isModalOpen && recipe.id && <MealPlanModal recipeId={recipe.id} onClose={() => setIsModalOpen(false)} onSave={() => showFeedback('Zum Essensplan hinzugefügt!')} />}
      ...
      <div className="bg-zinc-950/50 border border-zinc-800 rounded-lg shadow-xl p-6 md:p-8">
        <h2 className="text-3xl md:text-4xl font-bold text-amber-400 mb-4">{recipe.recipeTitle}</h2>
        <p className="text-zinc-300 mb-6">{recipe.shortDescription}</p>
        
        <div className="flex flex-wrap gap-x-6 gap-y-4 text-zinc-300 mb-6 pb-6 border-b border-zinc-700">
            <div className="flex items-center" title="Gesamtzeit"><Clock size={18} className="mr-2 text-zinc-500" /> {recipe.totalTime}</div>
            <div className="flex items-center" title="Portionen"><Users size={18} className="mr-2 text-zinc-500" /> {recipe.servings}</div>
            <div className="flex items-center" title="Schwierigkeit"><BarChart size={18} className="mr-2 text-zinc-500" /> {recipe.difficulty}</div>
        </div>
          ...
          <div className="my-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
              <div className="bg-zinc-800 p-3 rounded-lg">
                ...
                <p className="text-lg font-bold text-amber-400">{recipe.nutritionPerServing.calories}</p>
              </div>
              <div className="bg-zinc-800 p-3 rounded-lg">
                <p className="text-sm text-zinc-400">Protein</p>
                <p className="text-lg font-bold text-amber-400">{recipe.nutritionPerServing.protein}</p>
              </div>
              <div className="bg-zinc-800 p-3 rounded-lg">
                <p className="text-sm text-zinc-400">Fett</p>
                <p className="text-lg font-bold text-amber-400">{recipe.nutritionPerServing.fat}</p>
              </div>
              <div className="bg-zinc-800 p-3 rounded-lg">
                <p className="text-sm text-zinc-400">Kohlenhydrate</p>
                <p className="text-lg font-bold text-amber-400">{recipe.nutritionPerServing.carbs}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="mt-10 pt-6 border-t border-zinc-700 flex flex-col sm:flex-row justify-between items-center gap-4">
             ...
             <div className="relative inline-block">
                ...
                {isExportOpen && (
                    <div className="absolute bottom-full mb-2 w-full bg-zinc-800 border border-zinc-700 rounded-md shadow-lg z-10">
                        <a onClick={() => { exportRecipeToPdf(recipe); setExportOpen(false); }} className="block text-sm px-4 py-2 hover:bg-zinc-700 cursor-pointer">PDF</a>
                        <a onClick={() => { exportRecipeToCsv(recipe); setExportOpen(false); }} className="block text-sm px-4 py-2 hover:bg-zinc-700 cursor-pointer">CSV</a>
                        <a onClick={() => { exportRecipeToMarkdown(recipe); setExportOpen(false); }} className="block text-sm px-4 py-2 hover:bg-zinc-700 cursor-pointer">Markdown</a>
                        <a onClick={() => { exportRecipeToTxt(recipe); setExportOpen(false); }} className="block text-sm px-4 py-2 hover:bg-zinc-700 cursor-pointer">Text</a>
                    </div>
                )}
             </div>
           </div>
           ...
           <div className="flex items-center gap-4">
                {feedback && <p role="status" className="text-sm text-zinc-400">{feedback}</p>}
                 {isSaved && (
                    <button onClick={handleToggleFavorite} title={isFavorite ? "Favorit entfernen" : "Als Favorit markieren"} className="p-2 rounded-full text-zinc-400 hover:text-amber-400 hover:bg-zinc-700 transition-colors">
                        <Star size={22} className={`transition-colors ${isFavorite ? 'fill-current text-amber-400' : ''}`} />
                    </button>
                )}
                {isSaved ? (
