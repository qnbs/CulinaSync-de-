...
- **Coverage (M5 ✅, M5.8, Juni 2026):** ca. **79,6 %** stmts / **81,1 %** lines / **75,1 %** funcs / **64,0 %** branches (v8); Thresholds **80/78/73/64** in `apps/web/vitest.config.ts`. **470** Vitest-Tests / **109** Dateien + **`pnpm run test:scripts`**. **E2E:** **10** Tests / **6** Specs. **ESLint:** `no-floating-promises` (projectService). **CI:** `validate.yml`; Playwright **v1.60.0** in `e2e-smoke.yml`. Status: `docs/STATUS-2026-06-03.md`.
