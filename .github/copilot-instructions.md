...
- **Coverage:** Stand Mai 2026 ca. **42 %** Statements / **44 %** Lines (v8); Ziel M5: ≥70 %. **119** Tests / **34** Dateien; u. a.: `MealPlannerContext`, `PantryManagerContext`, `ShoppingListContext`, `useMealPlannerScreen`, `useMealPlan`, `useShoppingList`, `usePantryManager`, `useCookModeController`, Smoke MealPlanner/CookMode/RecipeDetailTabs/**PantryManager**/**ShoppingList** (`smokeHookStubs.ts`), `mealPlanRepository`/`pantryRepository`, `dayColumnPantryStatus`, `geminiMsw.test.ts` (Zod). CI laeuft `test:coverage` und laedt `coverage` als Artefakt.
