...
- **Coverage (M5.9 floor, September 2026):** Thresholds **82/80/75/82** (lines/stmts/funcs/branches) in `apps/web/vitest.config.ts`; **156** Vitest-Dateien + **`pnpm run test:scripts`**. **E2E:** **10** Tests / **6** Specs; Smoke Chromium blocking; Matrix Chromium+Firefox on PR/main, WebKit weekly/manual. **CI:** `validate.yml`; Playwright **v1.61.1** in `e2e-smoke.yml` / `e2e-matrix.yml`. Status: `docs/STATUS-2026-09-02.md`.
